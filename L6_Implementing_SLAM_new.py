...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    Omega = matrix()
    Xi    = matrix()
    Omega.zero((N + num_landmarks) * 2, (N + num_landmarks) * 2)
    Xi.zero((N + num_landmarks) * 2, 1)
    omgmap = [[0.0 for j in range((N + num_landmarks) * 2)] for i in range((N + num_landmarks) * 2)]
    omgmap[0][0] = 1.0
    omgmap[1][1] = 1.0
    Omega += matrix(omgmap)
    ximap = [[0.0] for i in range((N + num_landmarks) * 2)]
    ximap[0][0] = world_size / 2.0
    ximap[1][0] = world_size / 2.0
    Xi += matrix(ximap)
    for di, dt in enumerate(data):
        for z in dt[0]:
            omgmap = [[0.0 for j in range((N + num_landmarks) * 2)] for i in range((N + num_landmarks) * 2)]
            omgmap[di * 2][di * 2] += 1.0
            omgmap[di * 2 + 1][di * 2 + 1] += 1.0
            omgmap[(N + z[0]) * 2][(N + z[0]) * 2] += 1.0
            omgmap[(N + z[0]) * 2 + 1][(N + z[0]) * 2 + 1] += 1.0
            omgmap[di * 2][(N + z[0]) * 2] += -1.0
            omgmap[di * 2 + 1][(N + z[0]) * 2 + 1] += -1.0
            omgmap[(N + z[0]) * 2][di * 2] += -1.0
            omgmap[(N + z[0]) * 2 + 1][di * 2 + 1] += -1.0
            Omega += matrix(omgmap)
            ximap = [[0.0] for i in range((N + num_landmarks) * 2)]
            ximap[di * 2][0] += -z[1]
            ximap[di * 2 + 1][0] += -z[2]
            ximap[(N + z[0]) * 2][0] += z[1]
            ximap[(N + z[0]) * 2 + 1][0] += z[2]
            Xi += matrix(ximap)
        d = dt[1]
        omgmap = [[0.0 for j in range((N + num_landmarks) * 2)] for i in range((N + num_landmarks) * 2)]
        omgmap[di * 2][di * 2] += 1.0
        omgmap[di * 2 + 1][di * 2 + 1] += 1.0
        omgmap[(di + 1) * 2][(di + 1) * 2] += 1.0
        omgmap[(di + 1) * 2 + 1][(di + 1) * 2 + 1] += 1.0
        omgmap[di * 2][(di + 1) * 2] += -1.0
        omgmap[di * 2 + 1][(di + 1) * 2 + 1] += -1.0
        omgmap[(di + 1) * 2][di * 2] += -1.0
        omgmap[(di + 1) * 2 + 1][di * 2 + 1] += -1.0
        Omega += matrix(omgmap)
        ximap = [[0.0] for i in range((N + num_landmarks) * 2)]
        ximap[di * 2][0] += -d[0]
        ximap[di * 2 + 1][0] += -d[1]
        ximap[(di + 1) * 2][0] += d[0]
        ximap[(di + 1) * 2 + 1][0] += d[1]
        Xi += matrix(ximap)
    mu = Omega.inverse() * Xi
