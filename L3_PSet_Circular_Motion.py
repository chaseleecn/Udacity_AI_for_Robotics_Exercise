...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if motion[0] != 0:
            ...
            r = self.length / tan(motion[0])
            beta = motion[1] / self.length * tan(motion[0])
            self.x += 2 * r * sin(beta / 2) * cos(self.orientation + beta / 2)
            self.y += 2 * r * sin(beta / 2) * sin(self.orientation + beta / 2)
            self.orientation = (self.orientation + beta) % (2 * pi)

        else:
            self.x += motion[1] * cos(self.orientation)
